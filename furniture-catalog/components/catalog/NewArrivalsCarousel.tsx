...
export function NewArrivalsCarousel({ products }: { products: ProductWithRelations[] }) {
  ...
  return (
    <div className="relative group/carousel">
      {/* Стрелки — только на десктопе, на телефоне листают свайпом/точками */}
      {products.length > 1 && (
        <>
          <button
            type="button"
            ...
            className="hidden md:flex absolute -left-5 top-[38%] -translate-y-1/2 z-10 h-10 w-10 items-center justify-center rounded-full border border-line bg-bone shadow-sm hover:border-brass disabled:opacity-0 transition-opacity cursor-pointer"
          >
            ←
          </button>
          <button
            ...
            className="hidden md:flex absolute -right-5 top-[38%] -translate-y-1/2 z-10 h-10 w-10 items-center justify-center rounded-full border border-line bg-bone shadow-sm hover:border-brass disabled:opacity-0 transition-opacity cursor-pointer"
          >
            →
          </button>
        </>
